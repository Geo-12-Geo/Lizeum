...
class Board:
    ...
    def get_cell(self, mouse_pos):
        ...
        if cell_x < 0 or cell_x >= self.width or cell_y < 0 or cell_y >= self.\
           height:
            return None
    ...
    def next_move(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                for dy in range(-1, 2):
                    for dx in range(-1, 2):
                        if x + dx < 0 or x + dx >= self.\
                           width or y + dy < 0 or y + dy >= self.height:
                            continue
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and event.\
           key == pygame.K_SPACE or event.\
           type == pygame.MOUSEBUTTONDOWN and event.button == 3:
            time_on = not time_on
